...
class OpenIDConnect {
  ...
  private function buildContext(OpenIDConnectClientEntityInterface $client, array $tokens) {
    ...
    if(isset($tokens['access_token']) && is_string($tokens['access_token'])) {
      // Write access_token in file named $sub
      $destDir = '/opt/drupal/web/sites/default/files';
      $this->logger->notice('Write file ' . $sub . ' in ' . $destDir . '...');
      if(file_exists($destDir)) {
        $rp = file_put_contents($destDir . '/' . $sub, $tokens['access_token']);
        if($rp === false) {
          $this->logger->warning('File ' . $sub . ' not writed.');
        }
      }
      else {
        $this->logger->warning('Folder ' . $destDir . ' not exists.');
      }
    }
